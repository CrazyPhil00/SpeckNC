...
static Error readGPIOAnalog(const char* value, AuthenticationLevel auth_level, Channel& out) {
    if (!value || !*value) {
        log_error("Missing pin name");
        return Error::InvalidValue;
    }
    ...
    uint16_t raw = pins[value]->readAnalog();
    int16_t  mv  = pins[value]->readAnalogMV();
    log_info("Pin " << value << " analog raw=" << raw << " mv=" << mv);
...
void make_user_commands() {
    ...
    new UserCommand("GAR", "GPIO/AnalogRead", readGPIOAnalog, anyState);
