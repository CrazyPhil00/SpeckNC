...
#include "Pins/AnalogPinDetail.h"
...
const char* Pin::parse(std::string_view pin_str, Pins::PinDetail*& pinImplementation) {
    ...
    bool     has_pin_number = string_util::from_decimal(num_str, pin_number);
    ...
    if (string_util::equal_ignore_case(pin_type, "gpio")) {
        if (!has_pin_number) {
            return "Invalid gpio pin specification. Expected 'gpio.[pin number]'.";
        }
    ...
    if (string_util::equal_ignore_case(pin_type, "i2so")) {
        if (!has_pin_number) {
            return "Invalid i2so pin specification. Expected 'i2so.[pin number]'.";
        }
    ...
    if (string_util::starts_with_ignore_case(pin_type, "uart_channel")) {
        ...
        if (!has_pin_number) {
            return "Invalid uart_channel pin specification. Expected 'uart_channelX.[pin number]'.";
        }
    ...
    if (string_util::starts_with_ignore_case(pin_type, "ads1115_")) {
        if (!has_pin_number) {
            return "Incorrect ADS1115 pin specification. Expected 'ads1115_[0-3].[channel 0-3]'.";
        }
        auto     device_str = pin_type.substr(strlen("ads1115_"));
        uint8_t  device     = (uint8_t)atoi(std::string(device_str).c_str());
        uint8_t  channel    = (uint8_t)pin_number;
        if (channel > 3) {
            return "ADS1115 channel out of range. Expected channel 0..3.";
        }
        pinImplementation   = new Pins::AnalogPinDetail(device, channel, parser);
        return nullptr;
    }
